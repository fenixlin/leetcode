...
public:
    int maxArea(vector<int>& height) {
        ...
        for (int i=0; i<v.size(); i++) {
            if (min!=0x7fffffff && v[i].first*abs(v[i].second-min)>ans)
                ans = v[i].first*abs(v[i].second-min);
            if (max!=0x80000000 && v[i].first*abs(max-v[i].second)>ans)
                ans = v[i].first*abs(max-v[i].second);
